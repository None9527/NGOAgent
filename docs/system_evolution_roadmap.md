...
- orchestration 网络层已经完成基础接线，但需要统一 runtime 承载
- 域能力外化已经完成 closure pass，但需要严格边界与观测合同

这些问题在 `R1-R4` 主线收口后，已经从“路线阻塞”转为“后续改动必须持续守住的结构纪律”。
...
当前实现顺序并不是严格线性推进：`R3` 和 `R4` 的外围接线曾先于 `R1/R2` 收口完成。这个事实不推翻总路线，只说明外围层可以先整理边界；路线表达仍然必须尊重内核、合同、编排、外化的依赖关系。
...
这是 NGOAgent 成为 Agent Runtime 的起点，也是后续所有阶段的承载面。
...
这一阶段完成后，NGOAgent 不再是“带 graph 雏形的产品”，而是以 Graph Runtime 为主执行骨架。
...
但当前实现顺序已经不是严格线性推进：`R3` 和 `R4` 的外围接线曾先于 `R1/R2` 收口完成。这不是路线错误，而是说明外围层可以先整理边界；最终的完成判断仍然要回到 `R1` 内核所有权和 `R2` 智能合同化。

当前实际状态证明：

- `R3` 曾可以先完成 EventBus / A2A / AggregatedToolDiscovery 的外围接线
- `R4` 曾可以先完成 provider/source/discovery 边界和 assembly 收口
- 但 `R3/R4` 的完成不能反向证明 `R1/R2` 已经完成，必须单独验收内核与合同
- 现在 `AgentLoop` 不再定义主执行语义，Graph Runtime 已成为主执行骨架
- planning/reflection/evaluation 已进入统一 intelligence contract，R2 已完成主线收口

所以依赖关系仍然成立，并成为后续守护边界：
...
现在的风险不是 `R3/R4` 没做，而是在 `R1-R4` 已完成后又重新引回旧 loop 心智、兼容壳主导心智或边界混用。
...
| `R5 生产化` | ⚠️ 只能局部推进 | health/readiness 等生产化能力可以补，但必须建立在已完成的 R1/R2 收口之上 |
