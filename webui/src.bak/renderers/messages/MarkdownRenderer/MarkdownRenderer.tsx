...
export const MarkdownRenderer: FC<MarkdownRendererProps> = ({
  ...
  const segments = useMemo((): Segment[] => {
    try {
      ...
      // Step 3.5: Inject copy buttons into code blocks
      html = html.replace(/<pre>(\s*<code(?:\s+class="([^"]*)")?)/g, (_m, codeTag, langClass) => {
        const lang = langClass ? langClass.replace(/^language-/, '') : '';
        const label = lang ? `<span class="code-lang-label">${escapeHtml(lang)}</span>` : '';
        return `<pre class="code-block-wrapper">${label}<button type="button" class="code-copy-btn" title="复制代码">复制</button>${codeTag}`;
      });
