...
export default function App() {
  ...
  const handleRetry = useCallback(async () => {
    ...
    try {
      ...
      // Re-send through normal chat flow
      setTimeout(() => sendRef.current?.(undefined, last_message), 50)
    } catch (err) {
        ...
        <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-1 min-h-0 w-full overflow-y-scroll relative z-0" style={{ WebkitOverflowScrolling: 'touch' }}>
          ...
          <div className="w-full max-w-4xl mx-auto flex flex-col min-h-full pt-10 md:pt-20 px-1 md:px-4 relative">
            ...
            <div className="w-full flex flex-col relative">
              <ChatViewer messages={messages} theme="dark" sessionId={sessionId} onRetry={handleRetry} />

              {/* Thinking indicator: visible after user sends (streaming) or during auto-wake */}
              {(streamPhase === 'streaming' || streamPhase === 'auto_waking') && messages.length > 0 && messages[messages.length - 1].type === 'user' && (
