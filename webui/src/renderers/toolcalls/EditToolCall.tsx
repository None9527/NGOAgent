...
export const EditToolCall: React.FC<BaseToolCallProps> = ({
  ...
  // Fallback: text content only (e.g., history loaded messages)
  // Extract path from rawInput or parse from text like "Successfully edited /path/to/file"
  const { textOutputs } = groupContent(content);
  if (textOutputs.length > 0) {
    const text = textOutputs.join('\n');
    let fallbackPath = '';
    if (toolCall.rawInput && typeof toolCall.rawInput === 'object') {
      fallbackPath = (toolCall.rawInput as Record<string, unknown>).path as string || '';
    }
    if (!fallbackPath) {
      const pathMatch = text.match(/(?:edited|modified)\s+(\S+)/i);
  ...
  // No output at all, don't show anything
  return null;
};
