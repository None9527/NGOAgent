...
/**
 * Shell output card with expand/collapse for long output
 */
const ShellOutputCard: FC<{
  inputCommand: string;
  output: string;
  isLong: boolean;
  classPrefix: string;
  onInClick: () => void;
  onOutClick: () => void;
}> = ({ inputCommand, output, isLong: _isLong, classPrefix, onInClick, onOutClick }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  // CSS truncates at 60px height, so any multi-line output needs expand capability
  const hasContent = output.length > 0;
  const displayOutput = !isExpanded && output.length > 500 ? output.substring(0, 500) + '...' : output;
  // When expanded, add the '-toolcall-full' class to remove CSS max-height/mask
  const rowContentClass = `${classPrefix}-toolcall-row-content${isExpanded ? ` ${classPrefix}-toolcall-full` : ''}`;
  ...
  return (
    <div className={`${classPrefix}-toolcall-card`}>
      <div className={`${classPrefix}-toolcall-content`}>
        ...
        {hasContent && (
          <div className="flex justify-center border-t border-[var(--app-input-border)] pt-1 pb-1">
            <button
              type="button"
