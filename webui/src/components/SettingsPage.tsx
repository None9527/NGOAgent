...
function ReadOnlyRow({ label, val }: { label: string; val: any }) {
  return (
...
/* ════════════════ Seamless Inputs (Blur-to-save) ════════════════ */

function InputAutoSave({ label, val, configKey, type, onSave, status, placeholder }: {
  ...
  // Sync if external config changes while not focused
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col gap-1.5 relative">
      ...
      <div className="relative group">
        ...
        />
        {/* Status indicator injected inside input block */}
...
function NativeSwitch({ label, value, configKey, onSave }: { label: string, value: boolean, configKey: string, onSave: (k: string, v: any) => Promise<void> }) {
...
function TagEditor({ label, items, configKey, onSave, status }: { label: string, items: string[], configKey: string, onSave: (k: string, v: any) => Promise<void>, status?: string }) {
